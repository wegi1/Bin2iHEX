...
size_t address = 0x00000000;   // 32 bit record address - defauld address value = (0)
uint32_t addr_len;             // length of text address parameter (argv[3])
...
//======================================
static int show_help(int data);
static void check_prgname(void);
static int check_params(void);
static char U_Case(char code);
static void get_addres_value(void);
static void get_decimal_value(void);
static void get_hex_value(void);
...
static int write_records(void);
static void store_colon(void);
static int  store_byte(uint8_t data);
static void store_nline(void);
static uint8_t nybble_to_ASCII(uint8_t nybble);
static uint8_t byte_to_ASCII(uint8_t data);
static int make_seg04(void);
static int store_eof(void);
//===========================
...
int main(int argc, char* argv[]) {
	...
	get_addres_value();                            // chec address parameter - if error, use the default value
	end_add  = address + filestatus.st_size;       // end adress for statitics print
	addr_rec = address;                            // store records address
...
// as address and store address to variable
//=====================================================
static void get_decimal_value(void) {
	...
	address = new_add;                        // store new address value
}
//===
//=====================================================
...
static void get_addres_value(void) {
	...
	} // END ...  if(arg_count > 3) { // exist address parameter?
}
//===
...
static int read_data(void) {
	...
	if (rest_file < 65536) {             // set the value of bytes to read 
	...
	inbuf_rest = length_data_bytes;      // set the count of a data to read in the input buffer 
...
static uint8_t nybble_to_ASCII(uint8_t nybble) {
	uint8_t data;
	data = nybble & 0x0f;                 // trim to low 4 bits value (nybble)
...
static int write_records(void) {
	...
	} // END...  while((opened_input + opened_output) != 0)
...
static int store_eof(void) {
	int result;
	store_colon();           // start record marker ... eof record = ":", 0, 0, 0, 1, 0xFF, 0x0A
